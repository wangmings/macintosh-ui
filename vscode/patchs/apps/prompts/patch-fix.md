...
7. 这个补丁工具的目标是实现 activity bar / sidebar 相关功能；补丁数量不固定，如果新版源码变化导致原有补丁无法覆盖功能，可以在 `activityBar.patch` 中新增必要补丁对象
...
- 可以根据实际源码变化新增 `activityBar.patch` 补丁对象，但不要修改补丁执行方法、不要修改其它配置段
- 不要把字符串匹配改成正则
